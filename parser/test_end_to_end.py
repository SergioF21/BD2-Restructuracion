...
class TestEndToEnd(unittest.TestCase):
    ...
    def test_select_with_where_equals(self):
        ...
        # Crear tabla
        sql = f'CREATE TABLE TestSelect WHERE FROM FILE "{self.temp_csv}" USING INDEX BTree("id")'
        plan = self.parser.parse(sql)
        self.executor.execute(plan)
...
class TestEndToEndIntegration(unittest.TestCase):
    ...
    def _create_temp_csv(self):
        ...
        test_data = [
            ["id", "nombre", "precio"],
            [1, "Restaurante A", "25.50"],
            [2, "Restaurante B", "35.75"],
            [3, "Restaurante C", "18.25"],
            [4, "Restaurante D", "42.00"],
            [5, "Restaurante E", "28.90"]
        ]
